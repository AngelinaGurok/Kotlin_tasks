package kotlin_as_first.lesson5
...
fun buildGrades(grades: Map<String, Int>): Map<Int, List<String>> {
    val marksToStudents = hashMapOf<Int, MutableList<String>>()
    for((key, value) in grades){
        val keyForNewMap = value
        val valueForNewMap = key
        if(marksToStudents.get(keyForNewMap) == null){
            marksToStudents.put(keyForNewMap, mutableListOf(valueForNewMap))
        }
        else{
            val students = marksToStudents.get(keyForNewMap)
            students?.add(key)
            marksToStudents.set(keyForNewMap, students!!)
        }
    }
...
fun whoAreInBoth(a: List<String>, b: List<String>): List<String> {
    ...
    for(i in a.indices){
        if(b.contains(a[i]) && !inBoth.contains(a[i])){
            inBoth.add(a[i])
        }
    }
    return inBoth
...
fun mergePhoneBooks(mapA: Map<String, String>, mapB: Map<String, String>): Map<String, String> {
    ...
    for((key, value ) in mapB){
        if(mapAB[key] != null && mapB[key] != mapAB[key]){
            var newValue = mapAB[key]
            newValue = newValue + ", " + value
            mapAB.set(key, newValue)
        } else {
            mapAB.put(key, value)
        }
    }
...
fun averageStockPrice(stockPrices: List<Pair<String, Double>>): Map<String, Double> {
    ...
    for(i in stockPrices.indices){
        ...
        val price = stockPrices[i].second
        if(averagePrice.containsKey(key)){
            val price = (averagePrice[key]!! + price) / 2
            averagePrice.set(key, price)
        } else averagePrice.put(key, price)
    }
    return averagePrice
}
...
fun findCheapestStuff(stuff: Map<String, Pair<String, Double>>, kind: String): String? {
    ...
    for((key, value) in stuff){
        if(stuff.getValue(key).first == kind){
            if(minPrice == null || minPrice > stuff[key]!!.second){
...
fun extractRepeats(list: List<String>): Map<String, Int> {
    ...
    for(i in list.indices){
        ...
        if(repeats.containsKey(key)){
            val countRepeats = repeats[key]!! + 1
            repeats.set(key, countRepeats)
        } else repeats.put(key, 1)
    }
    val toRemove = mutableListOf<String>()
    for((key, value) in repeats){
        if(repeats.getValue(key) == 1){
            toRemove.add(key)
        }
    }

    for(i in toRemove.indices){
        repeats.remove(toRemove[i])
    }
